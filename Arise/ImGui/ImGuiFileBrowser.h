...
#ifndef ARISE_IMGUIFILEBROWSER_H
#define ARISE_IMGUIFILEBROWSER_H
...
inline ImGui::FileBrowser::FileBrowser(ImGuiFileBrowserFlags flags) : width_(700), height_(450), flags_(flags), openFlag_(false), closeFlag_(false), isOpened_(false), ok_(false),
...
inline void ImGui::FileBrowser::Display() {
    ...
    bool shouldExit = Button("cancel") || closeFlag_ || ((flags_ & ImGuiFileBrowserFlags_CloseOnEsc) && IsWindowFocused(ImGuiFocusedFlags_RootAndChildWindows) && escapeKey >= 0 && IsKeyPressed(escapeKey));
    if(shouldExit) {
        CloseCurrentPopup();
    }
...
#endif //ARISE_IMGUIFILEBROWSER_H
